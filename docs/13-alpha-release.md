...
公共包通过 Changesets 统一版本化并发布到 npm `@agentic-chat/*` scope。当前 Alpha 起始版本为 `0.1.0-alpha.0`，公共发布集合包括 core、runtime、react、react-ui、testkit、adapter-chatbi 和 adapter-ai-sdk；其中 adapter-ai-sdk 仍是尚未在 registry 首次创建的发布候选。私有的 `@agentic-chat/adapter-ag-ui` 和所有 `apps/*` 不发布。
