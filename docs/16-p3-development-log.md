...
- P3 的 snapshot/replay 等价验收项已完成；其余路线图项目仍需真实 adapter、UI 和恢复验收共同完成。

## 下一片入口

P3.3 将引入显式 attempt 语义、并行/父子 Activity selector 与 subagent 折叠 UI。retry 不复用旧 ToolCall/Activity ID，Task 与 Activity 继续通过显式 `activityId` 关联。
